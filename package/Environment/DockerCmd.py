...
class DockerFileSystem:
    ...
    def dockerWrite(cls, name, path, data):
        ...
        subprocess.run(f"docker exec -i {name} /bin/bash -c 'echo -e \"{data}\" > {path}'", shell=True)
...
class DockerContainer:
    ...
    def dockerCreate(cls, tag, name, port, volume):  # 建立docker container
        ...
        subprocess.run(f"docker create --name {name} -p {port} -v {volume} {tag}", shell=True)
    ...
    @classmethod
    def dockerExec(cls, name, cmd, detach=True, interactive=False, TTY=False):  # 執行docker container的指令
        par = "-" if interactive | detach | TTY else ""
        par += "i" if interactive else ""
        par += "d" if detach else ""
        par += "t" if TTY else ""
...
class dockerNetwork:
    ...
    @classmethod
    def dockerNetworkDisconnect(cls, name, container): # 將docker container從docker network移除
