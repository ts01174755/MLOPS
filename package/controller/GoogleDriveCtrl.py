...
class GoogleDriveBase():
    ...
    def create_folder(self, folder_name, parent_folder_id, print_file=False):
        try:
            ...
            file = service.files().create(body=file_metadata,
                                        fields='id').execute()
            if print_file: print(F'Folder ID: {file.get("id")}')
    ...
    # 上傳檔案
    def upload_file(self, target_file, file_name, mimetype='image/jpeg', folder_id=None, print_file=False):
        try:
            # create drive api client
            service = self._get_service()
            ...
            if print_file: print(F'File ID: {file.get("id")}')
        except HttpError as error:
            print(F'An error occurred: {error}')
            return False
        return True
    ...
    def delete_file(self, file_id, file_name, print_file=False):
        try:
            ...
            if print_file: print(F'File {file_name} has been deleted.')
        except HttpError as error:
            print(F'An error occurred: {error}')
            return False
        return True
    ...
    def delete_folder(self, folder_id, folder_name, print_file=False):
        try:
            ...
            if print_file: print(F'Folder {folder_name} has been deleted.')
        except HttpError as error:
            print(F'An error occurred: {error}')
            return False
        return True
...
class GoogleDrive(GoogleDriveBase):
    ...
    def _ls_cmd(self, targetFolder, print_file=True):
        ...
        if len(targetFolder) == 0:
            files = self.list_files(query="mimeType='application/vnd.google-apps.folder' and 'root' in parents")
            for file in files:
                if print_file: print(F'{file.get("name")}/')
    ...
    def _mkdir_cmd(self, targetFolder):
        ...
        for folder_ind_ in range(len(targetFolder)):
            ...
            if folder in self.folderIdTree[folderId]:
                ...
                # 建立資料夾
                file = self.create_folder(folder_name=folder, parent_folder_id=folderId, print_file=False)
                self.folderIdTree[folderId][folder] = file.get('id')
    ...
    def _upload_cmd(self, uploadFile, fileName, targetFolder, mimetype=None, print_file=True):
        # 取得資料夾ID
        folderId = 'root'
        for folder_ind_ in range(len(targetFolder)):
            if folderId not in self.folderIdTree:
        ...
        # 上傳檔案
        self.upload_file(target_file=uploadFile, file_name=fileName, mimetype=mimetype, folder_id=folderId,
    ...
    def _mv_cmd(self, sourceFile, targetFolder, print_file=True):
        ...
        sourceFileId = None
        files = self.list_files(query=F"mimeType!='application/vnd.google-apps.folder' and '{folderId}' in parents")
        for file in files:
        ...
        # 移動檔案
        self.move_file(file_id=sourceFileId, folder_id=folderId, file_name=fileName, print_file=print_file)
        return 'success'
    ...
    def _rmdir_cmd(self, sourceFile, print_file=True):
        ...
        # 刪除資料夾
        self.delete_folder(folder_id=folderId, folder_name=sourceFile[-1], print_file=print_file)
        return 'success'
    ...
    def _cp_cmd(self, sourceFile, targetFile, print_file=True):
        ...
        # 複製檔案
        self.copy_file(file_id=sourceFileId, file_name=fileName, folder_id=folderId, print_file=print_file)
    ...
    def execute_shell_command(self, command, print_file=True):
        ...
        if cmdList[0] == 'ls':
            if len(cmdList) == 1:
                ...
                if targetFolder[0] == '': targetFolder = targetFolder[1:]
            ...
            if len(cmdList) == 3:
                ...
                if sourceFile[0] == '': sourceFile = sourceFile[1:]
            ...
            if len(cmdList) == 2:
                ...
                if cmdList[1] == '-r':
                    ...
                    if sourceFile[0] == '': sourceFile = sourceFile[1:]
                    return self._rm_cmd(sourceFile=sourceFile, recursive=True, print_file=print_file)
                else:
                    print('rm指令參數錯誤')
        elif cmdList[0] == 'cp':
            if len(cmdList) == 3:
                ...
                if sourceFile[0] == '': sourceFile = sourceFile[1:]
                if targetFile[0] == '': targetFile = targetFile[1:]
                return self._cp_cmd(sourceFile=sourceFile, targetFile=targetFile, print_file=print_file)
            else:
                print('cp指令參數錯誤')
